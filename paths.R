...
download_tiger_files <- function(year, dir) {
  message("Attempting to download the ", year, " TIGER/Line Shapefiles...")
  placeholder <- "https://www2.census.gov/geo/tiger/TIGER%1$s/COUNTY/tl_%1$s_us_county.zip"
  url <- sprintf(placeholder, year)
  zip_file <- file.path(dir, "counties.zip")
  response <- GET(url, write_disk(zip_file, overwrite = TRUE))
  if (status_code(response) == 200) {
    message(year, " TIGER/Line Shapefiles downloaded.")
    unzip(zip_file, exdir = dir)
  }
}

process_tiger_files <- function(dir) {
  message("Attempting to extract county-level identifiers...")
  shp_file <- list.files(dir, pattern = "\\.shp$", full.names = TRUE)
  counties <- st_read(shp_file) %>%
    select(GEOID, STATEFP, COUNTYFP, NAME, NAMELSAD) %>%
    st_drop_geometry()
  message("County-level identifiers extracted.")
  return(counties)
}
...
extract_chr_data <- function(year, dir) {
  message("Attempting to extract the ", year, " County Health Rankings National Data...")
